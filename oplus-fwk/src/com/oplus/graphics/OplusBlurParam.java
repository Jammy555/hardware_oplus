...
public class OplusBlurParam {
    ...
    private static final int PARAMS_SIZE = 18;
    ...
    public void setMaterialParams(int mode, float[] blendC, float[] mixC) {
        mBlendMode = mode;
        System.arraycopy(blendC, 0, mBlendColor, 0, Math.min(blendC.length, mBlendColor.length));
        System.arraycopy(mixC, 0, mMixColor, 0, Math.min(mixC.length, mMixColor.length));
    }

    public void setArcylicParams(float[] arcylicColor) {
        System.arraycopy(
                arcylicColor, 0, mBlurColor, 0, Math.min(arcylicColor.length, mBlurColor.length));
    }
    ...
    public void setSmoothCornerWeight(float weight) {
        ...
        mSmoothCornerWeight =
                CORNER_TYPE == 0 ? weight : OplusSmoothRoundedManager.getDefaultG2Weight();
    }

    public float getSmoothCornerWeight() {
        return CORNER_TYPE == 0 ? mSmoothCornerWeight : OplusSmoothRoundedManager.getDefaultG2Weight();
    }
    ...
    public void setSmoothCornerType(int smoothCornerType) {
        mSmoothCornerType = smoothCornerType;
    }
    ...
    public float[] toFloatArray() {
        return new float[] {
                mBlurType, mTileMode, mZoomFactor, mBlendMode,
                ...
                mSmoothCornerWeight, mSmoothCornerType
        };
    }

    public void fromFloatArray(float[] floatArray) {
        if (floatArray.length != PARAMS_SIZE) {
    ...
    public int hashCode() {
        return Objects.hash(
                mBlurType, mTileMode, mZoomFactor, mBlendMode,
                Arrays.hashCode(mBlendColor),
                Arrays.hashCode(mMixColor),
                Arrays.hashCode(mBlurColor),
                mSmoothCornerWeight,
                mSmoothCornerType);
    }
}
